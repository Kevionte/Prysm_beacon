...
# github.com/Kevionte/prysm_beacon/v2/proto/prysm/v1alpha1
# ------------------------------------------------------
...
for ((i = 0; i < ${#proto_mocks_v1alpha1[@]}; i++)); do
    ...
    echo
    GO11MODULE=on mockgen -package=mock -destination="$file" github.com/Kevionte/prysm_beacon/v2/proto/prysm/v1alpha1 "$interfaces"
done
...
for ((i = 0; i < ${#iface_mocks[@]}; i++)); do
    ...
    echo "generating $file for interfaces: $interfaces";
    GO11MODULE=on mockgen -package=validator_mock -destination="$file" github.com/Kevionte/prysm_beacon/v2/validator/client/iface "$interfaces"
done

goimports -w "$mock_path/."
gofmt -s -w "$mock_path/."

# github.com/Kevionte/prysm_beacon/v2/validator/client/beacon-api
# -------------------------------------------------------------
