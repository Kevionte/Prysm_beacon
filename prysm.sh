...
function get_prysm_version() {
    if [[ -n ${USE_PRYSM_VERSION:-} ]]; then
        ...
        prysm_version=$(curl -f -s https://prysmaticlabs.com/releases/latest) || (color "31" "Starting prysm requires an internet connection. If you are being blocked by your antivirus, you can download the beacon chain and validator executables from our releases page on Github here https://github.com/Kevionte/prysm_beacon/releases/" && exit 1)
